...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  window.App = {
    ...
    async placeOrder(paymentMethod = 'cash') {
      ...
      try {
        const newOrder = await window.Api.orders.create(
          { restaurant_id: this.restaurant?.id, branch_id: this.branch?.id, type: 'dine_in', status: 'new', subtotal, total },
          this.cart.map(i => ({ product_id: i.id, name: i.name, price: i.price, quantity: i.qty }))
        );
