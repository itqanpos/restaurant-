// =============================================
// نظام المطاعم - Restaurant SaaS (آمن، مُصلح)
// =============================================

(async function() {
  // انتظر حتى تصبح مكتبة Supabase جاهزة
  ...
  window.App = {
    ...
    // ★ checkSession مُحاطة بمعالجة أخطاء كاملة
    async checkSession() {
      try {
    ...
    async loadTenantData() {
      if (!this.user) return;
      try {
        const { data, error } = await supabase.from('user_restaurant_roles')
