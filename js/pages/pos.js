...
// ★ تعريف دالة تحديث السلة عالمياً ★
window._updateCartDisplay = function() {
  const container = document.getElementById('cartItems');
  if (!container) return;

  if (!App.cart || !App.cart.length) {
...
// وظيفة التهيئة (تنتظر App جاهزاً ثم تربط الأزرار)
async function initPOS() {
  // انتظر حتى تصبح دوال App.addToCart جاهزة
  let attempts = 0;
  ...
  if (typeof App.addToCart !== 'function') {
    console.error('دوال السلة غير متوفرة');
    return;
  }
  ...
  // تحميل المنتجات (اختياري - لا يمنع الأزرار من العمل)
  try {
...
// ★★★ ربط الدوال العالمية (لكي تعمل أزرار onclick) ★★★
window.setOrderTypeLocal = setOrderType;
window.holdOrderLocal = holdOrder;
window.recallOrderLocal = recallOrder;
window.openSplitBillModalLocal = openSplitBillModal;
window.applyDiscountLocal = applyDiscount;
window.openAddonModalLocal = openAddonModal;
window.updateCartDisplay = _updateCartDisplay;
window.placeOrder = placeOrder;
window.addToCartGlobal = App.addToCart; // للاستخدام المباشر
